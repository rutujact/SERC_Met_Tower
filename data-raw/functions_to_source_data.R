...
fun.get_minute_by_year <-
  function(year = year,
           from.copy.path = from.copy.path,
           old.years = old.years) {
    if (!require("pacman"))
    ...
    for (i in 1:length(years.on)) {
      ...
      if (dir.exists(old.dir1)) {
        file.rename(from = old.dir1,
                    to = paste0(from.copy.path, years.on[i], "/Min"))
      } else
        if (dir.exists(old.dir2)) {
          file.rename(from = old.dir2,
                      to = paste0(from.copy.path, years.on[i], "/Min"))
        }
      # list all minute files for a year
      all.files0 <- list.files(
        path =
      ...
      if (!dir.exists(to.path.year)) {
        dir.create(to.path.year)
      }
      # create minute subfolder
      to.path.min <-
        paste0("data-raw/downloaded_data/", years.on[i], "/Min")
      if (!dir.exists(to.path.min)) {
        dir.create(to.path.min)
      }
      # copy files
      file.copy(
        from = all.files0,
      ...
      # store all files for a year that are in the local copy folder
      all.files[[i]] <- list.files(
        path =
          to.path.min,
        pattern = (".txt"),
        recursive = FALSE,
    ...
    cols.to.read <- 24
    for (i in 1:length(years.on)) {
      # to monitor prograess within a year
      ...
      pb <- txtProgressBar(min = 0,
                           ...
                           style = 3)
      dat <-
        setNames(data.frame(matrix(ncol = cols.to.read, nrow = 0)), paste0("V", 1:cols.to.read))
      for (j in 1:total) {
      ...
      write.table(
        dat,
        paste0(
          "data-raw/minute_by_year/Minute_data_",
          years.on[i],
          ".txt"
        ),
        row.names = T
...
# -------------------------------
fun.minute_by_year_into_one_file <-
  function(year = year, old.years = old.years) {
    if (!require("pacman"))
      install.packages("pacman")
    library(pacman)
    ...
    if (old.years == TRUE) {
        ...
        met.new2 <-
          subset(
            ...
            select = c(
              ...
              # Precip
              "V6",
              # wind speed, at tower-top per Pat Neale
              "V11",
              # Bp, at tower-top per Pat Neale
              "V15",
              # Temp top of tower, forest floor sensor was bad and has incorrect measurements, apparently
              "V16",
              # RH top of the tower. Forest floor also available, but would be unrepresentative for canopy evaporation
              "V17"
            )
          )# Rs top of the tower. Forest floor also available, but would be unrepresentative
        colnames(met.new2) = c(
          "Year",
          "Julian",
          "Hour",
      ...
      save(met0,
           file = paste(
             "data-raw/minute_data_",
             paste(min(prev.years),
                   max(prev.years), sep = "-"),
             ".Rda",
             sep = ""
           ))
    ...
    for (i in curr.years) {
      ...
      met.new3 <- data.frame(apply(met.new3, 2, as.numeric))
      met.new4 <- met.new3 %>%
        select(c(
          "V3",
          # Year
      ...
      if (i == curr.years[1]) {
        ...
        met1 <- rbind.data.frame(met1, met.new4)
      }
    }
    met1 <- met1 %>% mutate_all(as.numeric)
    write.table(
      met1,
      file = paste0("data-raw/minute_data_", paste0(curr.years), ".Rda"),
      row.names = FALSE
    )
